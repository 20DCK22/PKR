...
void AutoFillInfo(studentinfo* students, const int size)
{
	for (size_t i = 0; i < size; i++)
	{
		...
		students[i].LastName = "Dacko";
		students[i].course = rand() % 5 + 1;
		students[i].Prysicsmark = rand() % 5 + 1;
		students[i].Mathmark = rand() % 5 + 1;
...
void Task1(studentinfo* students, int size)
{
	...
	cout << "Percentage of excellent students: " << ((float)((float)k / (float)size)) * 100. << "%" << endl;
}
void Task2(studentinfo* students, const int size)
{
	cout << "¹\t|LN\t|Course\t|Speciality\t|Ph\t|ThSbj\t|Maths" << endl;
	for (size_t i = 0; i < size; i++)
		if (students[i].Prysicsmark == 5)
		{
			...
			case 0:
			{
				cout << "\t|" << students[i].thirdsubject.programming;
				break;
			}
			case 1:
			{
				cout << "\t|" << students[i].thirdsubject.numericmethods;
				break;
			}
			default:
			{
